...
def main(params):
    ...
    m, n = len(initDict['W1']), len(initDict['W1'][0])
    for i in xrange(m):
        for j in xrange(n):
                low_W2, high_W2 = initDict['W1'].copy(), initDict['W1'].copy()
                low_W2[i][j] = initDict['W1'][i][j] - 10e-4
                high_W2[i][j] = initDict['W1'][i][j] + 10e-4
                lowDict = autograd.eval(opseq, mlp.my_xman.inputDict(W1=low_W2))
                highDict = autograd.eval(opseq, mlp.my_xman.inputDict(W1=high_W2))
                if highDict['loss'] != lowDict['loss']:
                    print highDict['loss']
                    print lowDict['loss']
                    print gradientDict['W1'][i][j]
